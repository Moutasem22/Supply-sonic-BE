...
namespace AppAPI.Controllers
{
    ...
    {
        private readonly IAuthenticationService _authenticationRep;
        private readonly GlobalFormat _globalFormat;
        public AuthenticationController(IAuthenticationService authenticationRep, GlobalFormat globalFormat)
        {
        ...
        public IActionResult ResendOTP()
        {
            ...
            var val = _authenticationRep.ResendOTP(HttpContext.Request.Headers["Authorization"].ToString().Replace("Bearer ", ""));
            return Ok(val);
        }
        [HttpPost("Verify")]
        [Authorize(Policy = "LoginOtp")]
        public ActionResult<ResultViewModel<UserInfo>> Verify([FromBody] VerifyDto model)
        {
            ...
            var lang = string.IsNullOrWhiteSpace(HttpContext.Request.Headers["lang"]) ? "ar" : HttpContext.Request.Headers["lang"].ToString();
            var result = _authenticationRep.Verify(model.VerificationCode, claims, model.RemoteIpAddress);
            return Ok(result);
        }
