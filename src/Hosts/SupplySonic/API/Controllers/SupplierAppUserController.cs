...
[ApiController]
[Authorize]
public class SupplierAppUserController : ControllerBase
{
    private readonly ISupplierAppUserService _repository;
    IUserSettingService _userSettingRep;
    private readonly GlobalFormat _globalFormat;

    public SupplierAppUserController(ISupplierAppUserService repository, IUserSettingService userSettingRep, GlobalFormat globalFormat)
    {
    ...
    [HttpGet("GetOneAdmin/{UniqueId}")]
    public async Task<IActionResult> GetOneAdmin(string UniqueId)
    {
        var Id = int.Parse(DTO.EncryptionHelper.DecryptFromUrl(UniqueId));

        var val = await _repository.GetoneAdmin(Id);
    ...
    public async Task<IActionResult> AddUser(SupplierAppUserAddEditDto userdto)
    {
    ...
    public async Task<IActionResult> UpdateUser(SupplierAppUserAddEditDto userdto)
    {
        var val = await _repository.Update(userdto);
        return Ok(val);
    }
    [HttpPost("UpdateProfile")]
    public async Task<IActionResult> UpdateProfile(SupplierAppUserAddEditDto userdto)
    {
    ...
    public async Task<IActionResult> AddUserAddress(SupplierAppUserAddEditDto userdto)
    {
        var val = await _repository.AddUserAddress(userdto);
        return Ok(val);
    }
    ...
    public async Task<IActionResult> UpdateUserStatus(SupplierAppUserAddEditDto userdto)
    {
        var val = await _repository.UpdateStatus(userdto);
        return Ok(val);
    }
}
