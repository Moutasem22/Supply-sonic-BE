...
namespace Service
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public AuthenticationService(IBaseService baseService, IValidator<PasswordDto> passwordValidator, IValidator<SetPasswordDto> setPasswordValidator, IValidator<ResetPasswordDto> resetPasswordValidator, IOptions<AppSettings> options,
            ITokenFactory tokenFactory, IJwtTokenValidator jwtTokenValidator,IHostEnvironment env , INotificationService notificationService, SignInManager<AppUser> signInManager, IPasswordHasher<AppUser> passwordHasher)
        {
        ...
        public ResultViewModel<UserInfo> Authenticate(string username, string password, string RemoteIpAddress)
        {
            ...
            var user = _dbcontext.Users.Include(x => x.RefreshTokens).Include(x => x.ProfileAttachment).FirstOrDefault(x => (x.UserName == username || x.Email == username) && !x.IsDeleted && x.IsActive);
            if (user == null)
            {
                ErrorMessageDto errorMessage = new ErrorMessageDto() { ErrorMessage = "InvalidUsernameorPasswordMsg", PropertyName = "" };
                _baseService.GetExceptionMessages().ReturnExceptionMessages(errorMessage);
        ...
        public ExtrnalToken AuthenticateExternalSystem(string username, string password)
        {
            ...
            var user = _dbcontext.Users.FirstOrDefault(x => (x.UserName == username || x.Email == username) && x.IsDeleted != true);
        ...
        public ResultViewModel<UserInfo> Verify(string verificationCode, IEnumerable<Claim> claims, string RemoteIpAddress)
        {
            ...
            if (!VerifyOtp(otp_id, verificationCode, otp_hash))
            {
            ...
            var user = _dbcontext.Users.Include(x => x.RefreshTokens).FirstOrDefault(x => x.Id == userIdResult && x.IsDeleted != true);
            user.UpdateToken(GetVerifiedToken(new UserDTO()
            {
                Id = user.Id,
                UserName = user.UserName
        ...
        public ResultViewModel<UserInfo> GetAccessTokenUsingRefreshToken(string OldAccessToken, string RefreshToken)
        {
            ...
            try
            {
                ...
                if (cp != null)
                {
                    ...
                    var user = _dbcontext.Users.Include(x => x.RefreshTokens).FirstOrDefault(x => x.Id == UserId);
        ...
        public UserWithPagesDTO UserInfoAndPages(int userId, string currentPageCode = "")
        {
            ...
            {
                IsSuccess = true,
                Data = qpc.ToList().OrderBy(s=>s.Index).Adapt<List<PageCategoryDTO>>(),//_mapper.Map<List<PageCategoryDTO>>(qpc.ToList()),
                Total = qpc.Count()
            };
            ...
            UserWithPagesDTO.Date = DateTime.UtcNow;
        ...
        public UserDTO getItem(int Id)
        {
            ...
            var res = _dbcontext.Users
                ...
                .ThenInclude(c => c.Role).FirstOrDefault(x => x.Id == Id);

            var userRole = res.UserRoles.FirstOrDefault(a => _dbcontext.Roles.Any(s => s.IsMaster == true && a.RoleId == s.Id));
        ...
        public ResultViewModel<UserInfo> ResendOTP(string Token)
        {
            var result = new ResultViewModel<UserInfo>();
        ...
        public ResultViewModel<List<UserResultDto>> GetAllUsersForNotification(QueryViewModel<UserResultDto> queryViewModel)
        {
            ...
            var query = _dbcontext.Users.Where(r => r.IsActive == true && r.IsDeleted != true) as IEnumerable<AppUser>;
        ...
        public bool CheckUserIfSuperAdmin(string UserId)
        {
            ...
            var query = _dbcontext.Users.Where(r => r.IsActive == true && r.IsDeleted != true && r.Id == int.Parse(UserId)).FirstOrDefault();
        ...
        public ResultViewModel<bool> ForgetPassword(string email)
        {
            ...
            var user = _dbcontext.Users.FirstOrDefault(x => (x.Email == email) && x.IsDeleted != true);
            if (user == null)
            {
                ErrorMessageDto errorMessage = new ErrorMessageDto() { ErrorMessage = "InvalidEmail", PropertyName = "" };
                _baseService.GetExceptionMessages().ReturnExceptionMessages(errorMessage);
        ...
        public ResultViewModel<bool> SetPassword(SetPasswordDto dto)
        {
            ...
            var decodedToken = DecodeToken(token.Replace("Bearer ", ""));
            ...
            var user = _dbcontext.Users.FirstOrDefault(x => x.Id == int.Parse(userId) && x.IsDeleted != true);
            if (user == null)
            {
                ErrorMessageDto errorMessage = new ErrorMessageDto() { ErrorMessage = "InvalidUser", PropertyName = "" };
                _baseService.GetExceptionMessages().ReturnExceptionMessages(errorMessage);
