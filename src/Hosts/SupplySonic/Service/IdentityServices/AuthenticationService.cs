...
public class AuthenticationService : IAuthenticationService
{
    ...
    public AuthenticationService(IBaseService baseService, IValidator<PasswordDto> passwordValidator, IValidator<SetPasswordDto> setPasswordValidator, IValidator<ResetPasswordDto> resetPasswordValidator, IOptions<AppSettings> options,
        ITokenFactory tokenFactory, IJwtTokenValidator jwtTokenValidator, IHostEnvironment env, INotificationService notificationService)
    {
    ...
    public ResultViewModel<UserInfo> Authenticate(string username, string password, string RemoteIpAddress)
    {
        ...
        var user = _dbcontext.Users.Include(x => x.RefreshTokens).Include(x => x.ProfileAttachment).FirstOrDefault(x => (x.UserName == username || x.Email == username) && x.IsDeleted != true);
        if (user == null)
        {
            ErrorMessageDto errorMessage = new ErrorMessageDto() { ErrorMessage = "InvalidUsernameorPasswordMsg", PropertyName = "" };
            _baseService.ExceptionMessages.ReturnExceptionMessages(errorMessage);
    ...
    string GetForgetPasswordToken(UserDTO userDto)
    {
  ...
  public  bool CheckUserIfSuperAdmin(string UserId)
    ...
    string GeneratOTP(AppUser user)
    {
        var digit = 4;
    ...
    string GetVerifiedToken(UserDTO userDto)
    {
        var tokenHandler = new JwtSecurityTokenHandler();
        var key = Encoding.ASCII.GetBytes(_appSettings.Secret);
        var tokenDescriptor = new SecurityTokenDescriptor
    ...
    string GetGuestToken(UserDTO userDto, string otpCode)
    {
        var tokenHandler = new JwtSecurityTokenHandler();
        var key = Encoding.ASCII.GetBytes(_appSettings.Secret);
    ...
    string GetExternalSystemToken(UserDTO userDto)
    {
        var tokenHandler = new JwtSecurityTokenHandler();
        var key = Encoding.ASCII.GetBytes(_appSettings.Secret);
