...
namespace Service.IdentityServices
{
    public class RoleService : IRoleService
    {
        ...
        public ResultViewModel<RoleResultDto> Add(RoleAddEditDto roleDto)
        {
            ...
            var context = new ValidationContext(roleDto, null, null);
           //var isValid = Validator.TryValidateObject(roleDto, context, results, true);
            var Cresult = Validate(roleDto);
            ...
            //}
            roleDto.Permissions = new List<PermissionAddEditDto>();
            roleDto.PageActions.ToList().ForEach(x =>
            {
                var pageaction = _baseService.Context.PageActions.Where(y => y.ActionId == x.ActionId && y.PageId == x.PageId).FirstOrDefault();

                if (x.selected == true && pageaction != null)
                {
                    var permesstion = new PermissionAddEditDto
                    {
                        PageActionId = pageaction.Id
                    };
                    roleDto.Permissions.Add(permesstion);
                }
            });

            var role = new Role(roleDto.NameAr, roleDto.Name, roleDto.Permissions.Select(x => new Permission() { PageActionId = x.PageActionId }).ToList());

            _baseService.Context.Roles.Add(role);
            _baseService.Context.SaveChanges();


            _baseService.sendActionNotification(EnumPageCode.Role, EnumActionCode.add, role.Id);
        ...
        public ResultViewModel<RoleResultDto> Update(RoleAddEditDto roleDto)
        {
            ...
            var Cresult = Validate(roleDto);
            results.AddRange(Cresult);
            var role = _baseService.Context.Roles.FirstOrDefault(x => x.Id == roleDto.Id);
            ...
            roleDto.PageActions.ToList().ForEach(x =>
            {
                ...
                {
                    var permesstion = new PermissionAddEditDto();
                    permesstion.PageActionId = pageaction.Id;
                    roleDto.Permissions.Add(permesstion);
                }
            });


            role.Update(roleDto.NameAr, roleDto.Name, roleDto.Permissions.Select(x => new Permission() { PageActionId = x.PageActionId }).ToList());
            _baseService.Context.SaveChanges();

            _baseService.sendActionNotification(EnumPageCode.Role, EnumActionCode.update, role.Id);
